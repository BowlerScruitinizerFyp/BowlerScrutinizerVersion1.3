...
//    DatabaseReference mDatabaseRef;
    ...
    //for storage of image and angle in databae
...
//    }
    ...
    //To upload image with Angle in firebase
    private void uploadFile() {

        if(ImageUri != null )
        {

            StorageReference fileReference = mStorageRef.child(mAuth.getUid()).child("Image")
            ...
            .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                                                 ...
                                                 public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                                     ...
                                                     Toast.makeText(AnalyseActivity.this, "Upload successful", Toast.LENGTH_LONG).show();

                                                     String result = String.format("%.2f",theta);

                                                     String filename = "Angle = "+result;

                                                     ImageUpload imageUpload = new ImageUpload(filename.toString().trim() ,
                                                             taskSnapshot.getDownloadUrl().toString());

                                                     String uploadId = mDatabase.push().getKey();

                                                     //Store image with angle in database
                                                     mDatabase.child(mAuth.getUid()).child("Images").child(uploadId).setValue(imageUpload);
...
//                        setView(vew).setCancelable(false);
//
//            }
//        });


        final String finalTheta = String.format("%.2f" , theta);
        builder.setMessage("Angle found").
                setView(vew)
                .setPositiveButton("Save", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        String Result = finalTheta;
                        if(Result != null) {

                            //To upload Image with theta in firbase

                            if (mUploadTask != null && mUploadTask.isInProgress()) {
                                Toast.makeText(AnalyseActivity.this, "Upload in progress", Toast.LENGTH_SHORT).show();
                            }
